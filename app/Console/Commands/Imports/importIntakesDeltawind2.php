...
class importIntakesDeltawind2 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'import:importIntakesDeltawind2';
...
//        Loop door tabel tmp.
    ...
    $contact_emails = DB::table('email_addresses')
        ->where('email', $email)
...
//        Log::info('Toevoegen Contact B: ' . $contact_id . ' - Emailadres: ' . $contact_email);
        ...
        if ($contact_id == 0 && count($contact_emails) > 1) {
            ...
            foreach ($contact_emails as $contact_email) {
                if ($contact_email->contact_id != $saveContactId){
                    $doIntake = false;
                    break;
                }
            }
