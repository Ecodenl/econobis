...
class conversionProjectRevenuesKwh_Save extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'project:conversionProjectRevenuesKwhSave';
...
//            Log::error("komen we hier ???");
            $this->createStartRevenuePartsKwhNotConfirmedAndWithSplit($newRevenuesKwh, $hasKwhEndCalendarYear, $oldProjectRevenue);
        }else{
            $this->createStartRevenuePartsKwhIfConfirmedOrWithoutSplit($newRevenuesKwh, $hasKwhEndCalendarYear, $oldProjectRevenue);
        }
    ...
    protected function createStartRevenuePartsKwhNotConfirmedAndWithSplit(RevenuesKwh $revenuesKwh, $hasKwhEndCalendarYear, $oldProjectRevenue): void
    {
        $splitDates = [];
        ...
        $this->createPeriodParts($splitDates, $dateEndFromRevenue, $revenuesKwh, $oldProjectRevenue, true);
    }

    protected function createStartRevenuePartsKwhIfConfirmedOrWithoutSplit(RevenuesKwh $revenuesKwh, $hasKwhEndCalendarYear, $oldProjectRevenue): void
    {
        $splitDates = [];
        $dateBeginFromRevenue = Carbon::parse($revenuesKwh->date_begin)->format('Y-m-d');
        $splitDates[] = $dateBeginFromRevenue;
        ...
        $this->createPeriodParts($splitDates, $dateEndFromRevenue, $revenuesKwh, $oldProjectRevenue, false);
    }
     ...
     * @return array
     */
    protected function createPeriodParts(array $splitDates, string $dateEndFromRevenue, RevenuesKwh $revenuesKwh, $oldProjectRevenue, $withSplit): array
    {
        ...
        foreach ($periodParts as $periodPart) {
            $newRevenuePartsKwh = RevenuePartsKwh::create([
            ...
            if($withSplit){
                $getOldSplitRevenue = ProjectRevenue::where('project_id', $oldProjectRevenue->project_id)
                    ->whereNotNull('participation_id')
                    ->where('category_id', $this->projectRevenueKwhSplitCategoryId)
                    ->where('date_begin', '=',  $periodPart['startDate'])->first();
                if(!$getOldSplitRevenue) {
                    Log::error('Geen next split ?!?!');
                } else {
                    Log::info('Wel split');
                    Log::info('revenue_id ' . $revenuesKwh->id);
                    Log::info('date_registration ' . Carbon::parse($getOldSplitRevenue->date_end)->addDay()->format('Y-m-d'));
                    $revenueValuesKwh = RevenueValuesKwh::where('revenue_id', $revenuesKwh->id)->where('date_registration', Carbon::parse($getOldSplitRevenue->date_end)->addDay()->format('Y-m-d'))->first();
                    if ($revenueValuesKwh) {
                        if ($revenueValuesKwh->kwh_start != $getOldSplitRevenue->kwh_end
                            || $revenueValuesKwh->kwh_start_high != $getOldSplitRevenue->kwh_end_high
                            || $revenueValuesKwh->kwh_start_low != $getOldSplitRevenue->kwh_end_low) {
                            Log::info('Bestaande RevenueValuesKwh');

                            $revenueValuesKwh->kwh_start = $getOldSplitRevenue->kwh_end;
                            $revenueValuesKwh->kwh_start_high = $getOldSplitRevenue->kwh_end_high;
                            $revenueValuesKwh->kwh_start_low = $getOldSplitRevenue->kwh_end_low;
                            $revenueValuesKwh->save();
                            Log::error('Revenue kwh id: ' . $revenuesKwh->id . '. Verschillende beginstanden op ' . Carbon::parse($getOldSplitRevenue->date_end)->addDay()->format('d-m-Y') . '!!!!');
                        }
                    } else {
                        Log::info('Nieuwe RevenueValuesKwh');
                        Log::info('revenue_id ' . $revenuesKwh->id);
                        Log::info('date_registration ' . Carbon::parse($getOldSplitRevenue->date_end)->addDay()->format('Y-m-d'));

                        RevenueValuesKwh::create([
                            'revenue_id' => $revenuesKwh->id,
                            'date_registration' => Carbon::parse($getOldSplitRevenue->date_end)->addDay()->format('Y-m-d'),
                            'is_simulated' => false,
                            'kwh_start' => $getOldSplitRevenue->kwh_end ? $getOldSplitRevenue->kwh_end : 0,
                            'kwh_start_high' => $getOldSplitRevenue->kwh_end_high ? $getOldSplitRevenue->kwh_end_high : 0,
                            'kwh_start_low' => $getOldSplitRevenue->kwh_end_low ? $getOldSplitRevenue->kwh_end_low : 0,
                            'status' => $getOldSplitRevenue->confirmed ? 'confirmed' : 'concept',
                        ]);
                    }
                    if($getOldSplitRevenue->confirmed)
                    {
                        $newRevenuePartsKwh->confirmed = 1;
                        $newRevenuePartsKwh->date_confirmed = $getOldSplitRevenue->date_confirmed;
                        $newRevenuePartsKwh->status = 'confirmed';
...
//        $this->saveParticipantsOfDistribution($newRevenuesKwh, $oldProjectRevenuesKwh);
    ...
    public function addRevenueSplitPartsKwh($oldProjectRevenuesKwh, $newRevenuesKwh)
    {
        ...
        Log::info('newRevenuesKwh id: ' . $newRevenuesKwh->id);
        ...
        foreach ($splitRevenues as $splitRevenue){
            ...
            Log::info('splitDateString: ' . $splitDateString);
...
//                    }
            ...
            if($splitRevenue->confirmed == true){
                $revenuePartsKwh->confirmed = $splitRevenue->confirmed;
                $revenuePartsKwh->date_confirmed = $splitRevenue->date_confirmed;
                if($revenuePartsKwh->distributionPartsKwh->where('status', 'confirmed')->count() > 0)
                {
                    $revenuePartsKwh->status = 'confirmed';
                } elseif($revenuePartsKwh->distributionPartsKwh->where('status', 'processed')->count() > 0) {
                    $revenuePartsKwh->status = 'processed';
                } else {
                    $revenuePartsKwh->status = '?????????';
                }
            }
            $revenuePartsKwh->save();
            if ($revenuePartsKwh->confirmed) {
            ...
            $nextSplitRevenues = ProjectRevenue::where('project_id', $oldProjectRevenuesKwh->project_id)
                ->whereNotNull('participation_id')
                ->where('category_id', $this->projectRevenueKwhSplitCategoryId)
                ->where('date_begin', '=', $splitDateString);
            if($nextSplitRevenues->count()==0) {
                Log::error('Geen next split ?!?!');
            } elseif ($nextSplitRevenues->count()>1){
                Log::error('Meer dan 1  next split ?!?!');
            } else {
                $nextSplitRevenue = $nextSplitRevenues->first();
                Log::info($nextSplitRevenue);
                if($nextSplitRevenue->confirmed == true){
                    $newRevenuePartsKwh->confirmed = $nextSplitRevenue->confirmed;
                    $newRevenuePartsKwh->date_confirmed = $nextSplitRevenue->date_confirmed;
                    if($newRevenuePartsKwh->distributionPartsKwh->where('status', 'confirmed')->count() > 0)
                    {
                        $newRevenuePartsKwh->status = 'confirmed';
                    } elseif($revenuePartsKwh->distributionPartsKwh->where('status', 'processed')->count() > 0) {
                        $newRevenuePartsKwh->status = 'processed';
                    } else {
                        $newRevenuePartsKwh->status = '?????????';
                    }
                }
                $newRevenuePartsKwh->save();
                if ($newRevenuePartsKwh->confirmed) {
                    foreach ($newRevenuePartsKwh->conceptValuesKwh() as $conceptValueKwh) {
                        $conceptValueKwh->status = 'confirmed';
                        $conceptValueKwh->save();
                    }
                }
