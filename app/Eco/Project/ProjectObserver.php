...
class ProjectObserver
{
    ...
    public function saved(Project $project)
    {
        if ($project->isDirty('loan_type_id')) {
            // Als type lening veranderd is dan concept aflossing revenues op concept-to-update zetten.
            $projectRevenueCategoryRedemptionEuro = ProjectRevenueCategory::where('code_ref', 'redemptionEuro')->first()->id;
            $projectRevenues = $project->projectRevenues()->where('confirmed', false)->whereIn('status', ['concept', 'concept-to-update'])->whereIn('category_id', [$projectRevenueCategoryRedemptionEuro])->get();
