...
class HousingFileExcel2Helper
{
    private $housingFiles;

    public function __construct($housingFileSpecifications)
    {
        $housingFileIds = array_unique( $housingFileSpecifications->pluck('housing_file_id')->toArray() );
        Log::info($housingFileIds);

        $this->housingFiles = HousingFile::whereIn('id', $housingFileIds);
