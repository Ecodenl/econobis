...
abstract class RequestExtraFilter
{
    ...
    private function setRequestFilters(Request $request)
    {
        ...
        $mergedFilters = $this->mergeFilters($filters);
    ...
    private function mergeFilters($filters){
        ...
        for ($x = 0; $x < sizeof($filters)/3; $x++) {
            $mergedFilter = [
                'field' => $filters['field[' . $x . ']'],
                'type' => $filters['type[' . $x . ']'],
                'data' => $filters['data[' . $x . ']'],
            ];
            array_push($mergedFilters, $mergedFilter);
        }
