...
class TwinfieldSalesTransactionHelper
{
    ...
    public function __construct(Administration $administration)
    {
        ...
        $this->fromInvoiceDateSent = $this->getDateSync();
        ...
        $this->customerHelper = new TwinfieldCustomerHelper($this->administration, $this->connection);
    ...
    public function createSalesTransaction(Invoice $invoiceToProcess)
    {
        try {
            ...
            $twinfieldCustomer = $this->twinfieldCustomerCacheByContactId[$contactId] ?? null;

            if (!$twinfieldCustomer) {
                // Let op: pak direct het twinfieldNumber voor deze administratie
                $twinfieldNumber = $contact->twinfieldNumbers()
                    ->where('administration_id', $this->administration->id)
                    ->first();

                if ($twinfieldNumber && $twinfieldNumber->twinfield_number) {
                    // 1) Probeer customer op te halen
                    $twinfieldCustomer = $this->withRetry(
                        fn() => $this->customerHelper->getTwinfieldCustomerByCode($twinfieldNumber->twinfield_number, $contact)
                    );
                }
                ...
                if (!$twinfieldCustomer) {
                    ...
                    // createCustomer() kan response object of null/false teruggeven afhankelijk van jullie codepad
                    // Dus: daarna nog 1x ophalen als we een twinfieldNumber hebben
                    $twinfieldNumber = $contact->twinfieldNumbers()
                        ->where('administration_id', $this->administration->id)
                        ->first();
                    ...
                    if (!$twinfieldCustomer) {
                        // Stop nu: zonder customer code kun je geen sales transaction aanmaken
                        throw new \RuntimeException('Twinfield customer kon niet worden opgehaald/aangemaakt voor contact ' . $contact->number);
                    }
                }
    ...
    //            $descriptionDetail = $twinfieldCustomer ? ($twinfieldCustomer->getCode() . " " . $twinfieldCustomer->getName()) : ($invoiceToProcess->contact->number . " " . $invoiceToProcess->contact->full_name);
            ...
            $response = $this->withRetry(fn() => $this->transactionApiConnector->send($twinfieldSalesTransaction));
