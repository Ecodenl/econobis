...
class PortalSettingsController extends Controller
{
    ...
    public function update(PortalSettings $portalSettings, RequestInput $input, Request $request)
    {
        ...
        $data = $input->boolean('portalActive')->whenMissing(false)->onEmpty(false)->alias('portal_active')->next()
            ->string('portalName')->whenMissing(null)->onEmpty(null)->alias('portal_name')->next()
            ->string('cooperativeName')->whenMissing(null)->onEmpty(null)->alias('cooperative_name')->next()
            ->string('portalLoginInfoText')->whenMissing(null)->onEmpty(null)->alias('portal_login_info_text')->next()
            ->string('portalWebsite')->whenMissing(null)->onEmpty(null)->alias('portal_website')->next()
            ->string('portalUrl')->whenMissing(null)->onEmpty(null)->alias('portal_url')->next()
            ->integer('responsibleUserId')->whenMissing(null)->onEmpty(null)->alias('responsible_user_id')->next()
            ->integer('contactResponsibleOwnerUserId')->whenMissing(null)->onEmpty(null)->alias('contact_responsible_owner_user_id')->next()
            ->integer('checkContactTaskResponsibleUserId')->whenMissing(null)->onEmpty(null)->alias('check_contact_task_responsible_user_id')->next()
            ->integer('checkContactTaskResponsibleTeamId')->whenMissing(null)->onEmpty(null)->alias('check_contact_task_responsible_team_id')->next()
            ->integer('emailTemplateNewAccountId')->whenMissing(null)->onEmpty(null)->alias('email_template_new_account_id')->next()
            ->string('linkPrivacyPolicy')->whenMissing(null)->onEmpty(null)->alias('link_privacy_policy')->next()
            ->boolean('showNewAtCooperativeLink')->whenMissing(false)->onEmpty(false)->alias('show_new_at_cooperative_link')->next()
            ->string('newAtCooperativeLinkText')->whenMissing(null)->onEmpty(null)->alias('new_at_cooperative_link_text')->next()
            ->boolean('showAllowRequestForDelete')->whenMissing(false)->onEmpty(false)->alias('show_allow_request_for_delete')->next()
            ->string('allowRequestForDeleteButtonText')->whenMissing(null)->onEmpty(null)->alias('allow_request_for_delete_button_text')->next()
            ->double('pcrPowerKwhConsumptionPercentage')->whenMissing(null)->onEmpty(null)->alias('pcr_power_kwh_consumption_percentage')->next()
            ->double('pcrGeneratingCapacityOneSolorPanel')->whenMissing(null)->onEmpty(null)->alias('pcr_generating_capacity_one_solor_panel')->next()
            ...
            ->integer('defaultAdministrationId')->whenMissing(null)->onEmpty(null)->alias('default_administration_id')->next()
            ->get();
