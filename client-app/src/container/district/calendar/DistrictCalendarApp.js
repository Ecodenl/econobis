        ...
        DistrictAPI.fetchDistrictCalendarItems(props.params.id, startDate, endDate).then(data => {
            let quotationRequests = data.quotationRequests.map(item => {
                return {
                    ...
                    end: moment(item.datePlanned).add(item.durationMinutes, 'm').toDate(),
                };
            });
                    ...
                    <div className="col-md-4">
                        <div style={{display: 'flex', flexDirection: 'column'}}>
                            <div style={{display: 'flex'}}>
                                <InputSelect size={'col-12 col-sm-12'} label={'Coach'} name={'coach_id'} options={getCoaches()} value={filterCoachId} onChangeAction={(e) => setFilterCoachId(e.target.value)} placeholder="Alle" />
                            </div>
                            <div>
                                <InputToggle divSize={'col-12 col-sm-12'} label="Toon beschikbaarheid" name={'show_availabilities'} value={showAvailabilities} onChangeAction={(e) => setShowAvailabilities(e.target.checked)} />
                            </div>
                        </div>
                    </div>
                </div>
