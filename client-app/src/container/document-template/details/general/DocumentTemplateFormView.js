...
const DocumentTemplateFormView = props => {
    const {name, number, htmlBody, characteristic, roles, documentGroup, documentTemplateType, baseTemplate, headerTemplate, footerTemplate, active, createdAt, createdBy} = props.documentTemplate;
    return (
        <div>
            ...
            <div className="row" onClick={props.switchToEdit}>
                ...
                <ViewText
                    label={"Rollen"}
                    value={roles && roles.map((roles) => roles.name).join(', ')}
                />
                }
            </div>
