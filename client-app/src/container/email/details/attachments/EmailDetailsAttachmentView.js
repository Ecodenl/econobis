...
class EmailDetailsAttachmentView extends Component {
    ...
    render() {
        ...
        if (name.toLowerCase().endsWith('.pdf') || name.toLowerCase().endsWith('.jpg') || name.toLowerCase().endsWith('.png')) {
            hasValidExtension = true;
        }
        return (
            <div className={`row border ${this.props.highlightLine}`} onMouseEnter={() => this.props.onLineEnter()}
                ...
                {this.state.showViewDocument && (name.toLowerCase().endsWith('.png') || name.toLowerCase().endsWith('.jpg')) &&
                <Modal
                    closeModal={this.showViewDocument}
                    modalClassName='modal-lg'
                    modalMainClassName='email-attachment-modal '
