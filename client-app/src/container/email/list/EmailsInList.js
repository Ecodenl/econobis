    ...
    return (
        <div>
            <form onKeyUp={handleKeyUp}>
                <DataTable>
                    <DataTableHead>
                        <EmailsInListHead fetchEmailsData={() => props.fetchEmailsData()} />
                        <EmailsInListFilter folder={props.folder} onSubmitFilter={props.onSubmitFilter} />
                    </DataTableHead>

                    <DataTableBody>
                        {data.length === 0 ? (
                            <tr>
                                <td colSpan={9}>Geen e-mails gevonden!</td>
                            </tr>
                        ) : (
                            data.map(email => {
                                return (
