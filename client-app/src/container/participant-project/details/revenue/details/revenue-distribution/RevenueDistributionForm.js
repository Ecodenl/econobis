...
class RevenueDistributionForm extends Component {
    ...
    checkDistributionRevenueInvoices = () => {
        ...
        if (
            ...
            this.props.setError(
                412,
                'De uitkeringsdatum valt in jaar waar al een definitive waardestaat voor dit project aanwezig is.'
            );
            this.setState({
                showModal: false,
            });
        ...
        if (this.state.distributionIds.length) {
            if (!this.props.projectRevenue.project.administration.canCreatePaymentInvoices['can']) {
                ...
                this.setState({
                    showModal: true,
                    modalText:
                        'De uitkeringsdatum wordt de datum die bij de mutatie komt te staan in de deelname overzichten van de deelnemer.\n' +
                        '\n' +
    ...
    createPaymentInvoices = () => {
        ...
        const succesMessageText = `De mutaties van opbrengsten bij de deelnemers zijn aangemaakt. De status van de uitkeringen zijn veranderd van "Definitief" in Verwerkt.
