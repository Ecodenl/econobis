...
import TwinfieldListItem from './Item';
import TwinfieldLogAPI from '../../../api/twinfield/TwinfieldLogAPI';
...
import TwinfieldListFilter from './Filter';
import useKeyPress from '../../../helpers/useKeyPress';

const initialFilter = { createdAt: moment().format('YYYY-MM-DD'), value: '', messageType: null };

function TwinfieldListApp() {
    const [twinfieldLogs, setTwinfieldLogs] = useState([]);
    ...
    function fetchTwinfieldlogs() {
        setLoading(true);

        TwinfieldLogAPI.fetchTwinfieldLogs(formatFilterHelper(), sort, pagination)
            .then(payload => {
                setTwinfieldLogs(payload.data.data);
                ...
                <div className="margin-10-top">
                    <DataTable>
                        <DataTableHead>
                            ...
                            <TwinfieldListFilter filter={filter} handleChangeFilter={handleChangeFilter} />
                        </DataTableHead>
                        ...
                        <DataTableBody>
                            ...
                            ) : twinfieldLogs.length > 0 ? (
                                twinfieldLogs.map(twinfieldLog => {
                                    return <TwinfieldListItem key={twinfieldLog.id} {...twinfieldLog} />;
                                })
                            ) : (
                                <tr>
...
export default TwinfieldListApp;
