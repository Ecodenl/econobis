...
export function* fetchParticipantsProductionProjectSaga({filters, extraFilters,sorts, pagination, productionProjectId, filterType}) {
    try {
        //yield call(authSaga);
        yield put({ type: 'FETCH_PARTICIPANTS_PRODUCTION_PROJECT_LOADING' });
        const participantsProductionProject = yield call(ParticipantsProductionProjectAPI.fetchParticipantsProductionProject, {filters, extraFilters, sorts, pagination, productionProjectId, filterType});
