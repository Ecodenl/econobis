...
export function* fetchParticipantsProductionProjectSaga({filters, sorts, pagination, productionProjectId}) {
    try {
        //yield call(authSaga);
        yield put({ type: 'FETCH_PARTICIPANTS_PRODUCTION_PROJECT_LOADING' });
        const participantsProductionProject = yield call(ParticipantsProductionProjectAPI.fetchParticipantsProductionProject, {filters, sorts, pagination, productionProjectId});
