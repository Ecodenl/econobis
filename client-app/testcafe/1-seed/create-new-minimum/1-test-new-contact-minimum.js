...
test('Create new person', async t => {
    await t.useRole(superUser).navigateTo(constants.app_url + '#/contact/nieuw/persoon');
...
test('Check for title "Nieuw contact"', async t => {
    await t.useRole(superUser).navigateTo(constants.app_url + '#/contact/nieuw/organisatie');
