...
test('Fill out form housingfile', async t => {
    ...
    await t.click(personDetails.housingFileHarmonica).wait(constants.wait);

    await t.click(general.save).wait(constants.wait);

    await t
        .expect(general.titleH4.innerText)
        .eql('Woningdossier voor: ' + vars.personStreet + ' ' + vars.personNumber, 'Check element text', {
            timeout: 500,
        });
});
