...
test('Fill out form new production project -> participant -> value course', async t => {
    ...
    await t.expect(general.titleH3.innerText).eql('Nieuw project', 'Check element text', { timeout: 500 });
    ...
    await t
        ...
        .eql('Project ' + vars.projectName, 'Check element text', { timeout: 500 });

    await t
        .click(
            generalProject.rapportageButton
                .prevSibling()
                .prevSibling()
                .nth(1)
        )
    ...
    await t.expect(general.titleH3.innerText).eql('Nieuwe deelnemer', 'Check element text', { timeout: 500 });
    ...
    await t
        .expect(general.titleH4.innerText)
        .eql(vars.personLastName + ', ' + vars.personFirstName + '/' + vars.projectName, 'Check element text', {
            timeout: 500,
        });
    ...
    await t.click(generalProject.details).wait(constants.wait);

    await t.click(detailsProject.newValueCourse).wait(constants.wait);
